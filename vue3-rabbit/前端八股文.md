...
**2.toFixed()**，强制转化，四舍五入，得到字符串，**parseFloat()**再转化为数字

```javascript
parseFloat(sum.toFixed(10)) === 1
```
...
**2.**前端BigInt（es20,浏览器版本限制）

**3.**应该算方案，前端配置拦截器，出现大整数，将大数字转化为字符串
...
2.v-model :双向绑定，本质是props+emit的语法糖

3.Provide/Inject:跨层组件通信，

4.ref/defineExpose:

父组件通过ref获得子组件示例

子组件用defineExpose暴露方法给父组件
...
- **DOM**:broswer（渲染引擎）将html解析成树状对象结构（C++）,这个结构通过DOM API在js里面，表现为对象(首次渲染和全量更新)

js操作 dom :触发浏览器重排和重绘，性能开销大

- **Virtual DOM**:用js对象，描述真实DOM的轻量级副本

数据更新，diff算法，批量计算差异，只更新变化的部分到真实DOM
