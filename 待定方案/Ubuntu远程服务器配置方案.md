...
路由器管理地址（如 192.168.1.1）用于在浏览器中打开路由器的管理界面，进行网络配置。

### 2. 在配置Ubuntu远程服务器时的具体用途

#### 用途A：设置静态IP分配（重要）

- 问题：如果Ubuntu使用DHCP自动获取IP，重启后IP可能变化，导致SSH连接失败。

- 解决：在路由器中为Ubuntu的MAC地址绑定固定IP（如 192.168.1.100），这样每次获取的都是同一个IP。

操作位置：

- 路由器管理界面 → "DHCP服务器" → "静态IP分配" 或 "地址保留"

- 输入Ubuntu主机的MAC地址，绑定到固定IP

#### 用途B：配置端口转发（外网访问时）

- 如果需要从外网访问Ubuntu的SSH（端口22），需要在路由器中设置端口转发。

- 例如：外网访问路由器的22端口 → 转发到Ubuntu的 192.168.1.100:22

#### 用途C：配置DDNS（外网访问时）

- 如果家庭宽带是动态公网IP，可在路由器中配置DDNS，获得一个固定域名。

#### 用途D：查看当前设备

- 查看Ubuntu主机当前获取的IP地址，确认是否在线。
...
路由器 (192.168.1.1) ← 这是管理地址

 ↓

 ├─ Windows主机 (192.168.1.50)

 └─ Ubuntu主机 (192.168.1.100) ← 需要固定这个IP

如果不配置静态IP：

- 今天Ubuntu可能是 192.168.1.100

- 重启后可能变成 192.168.1.101

- 您用SSH连接时就会找不到主机

配置静态IP后：

- Ubuntu永远是 192.168.1.100

- 您可以用 ssh user@192.168.1.100 稳定连接

## 如何找到路由器管理地址

在Windows主机上打开CMD或PowerShell，执行：

ipconfig

查看"默认网关"那一行，就是路由器管理地址：

默认网关 . . . . . . . . . . . . : 192.168.1.1

然后在浏览器输入这个地址（如 http://192.168.1.1），输入管理员账号密码即可进入管理界面。
...
1. 给Ubuntu设置固定IP（避免IP变化导致连接失败）

1. 配置端口转发（外网访问时）

1. 配置DDNS（外网访问时）

如果只在内网使用，且Ubuntu已手动配置静态IP，可以跳过路由器配置。但通过路由器设置静态IP更简单可靠，推荐使用。
